...
locals {
  ...
  redis_host        = var.existing_redis_hostname != null ? var.existing_redis_hostname : (var.redis_vpe_enabled == true && (var.redis_service_endpoints == "public-and-private" || var.redis_service_endpoints == "private") ? data.ibm_database_connection.icd_redis_private_connection[0].redis[0].hosts[0].hostname : module.icd_redis.hostname)
  ...
  redis_port        = var.redis_vpe_enabled == true && (var.redis_service_endpoints == "public-and-private" || var.redis_service_endpoints == "private") ? data.ibm_database_connection.icd_redis_private_connection[0].redis[0].hosts[0].port : module.icd_redis.port
  ...
  redis_tls_cert_base64 = var.existing_redis_hostname != null ? null : (
    ...
    data.ibm_database_connection.icd_redis_private_connection[0].redis[0].certificate[0].certificate_base64 :
    module.icd_redis.service_credentials_object.credentials["tfe-redis"].connection.rediss.certificate.certificate_base64
  )
}
...
module "tfe_install" {
  count                     = module.ocp_vpc.cluster_id != null ? 1 : 0
  depends_on                = [module.icd_redis, module.icd_postgres_vpe, module.icd_redis_vpe]
